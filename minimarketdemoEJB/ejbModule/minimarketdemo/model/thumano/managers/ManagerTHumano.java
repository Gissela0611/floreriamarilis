...
import minimarketdemo.model.thumano.dtos.DTOThmRolPago;
...
public class ManagerTHumano {
	...
	public List<ThmEmpleado> findAllActivesEmployes() {
		List<ThmEmpleado> empleados = mDAO.execJPQL(
				"select te2 from ThmEmpleado t2, SegUsuario su where su.activo = true and te2.segUsuario.idSegUsuario = su.idSegUsuario");
		return empleados;
	}

	public List<DTOThmEmpleado> findAllDTOThmempleado() {
		List<DTOThmEmpleado> listaDTO = new ArrayList<DTOThmEmpleado>();
		List<ThmEmpleado> empleadoActivo = findAllActivesEmployes();
		DTOThmEmpleado e;
		DTOThmRolPago r;

		for (ThmEmpleado empleado : empleadoActivo) {
			e = new DTOThmEmpleado();
			e.setNombre(empleado.getSegUsuario().getNombres());
			e.setApellido(empleado.getSegUsuario().getApellidos());
			e.setCorreo(empleado.getSegUsuario().getCorreo());
			e.setNombreCargo(empleado.getThmCargo().getNombreCargo());
			e.setRemuneracionMensual(empleado.getThmCargo().getRemuneracionMensual().doubleValue());
			for(ThmRolCabecera rol:empleado.getThmRolCabeceras()) {
				r = new DTOThmRolPago();
				r.setPeriodoRol(rol.getPeriodoRol());
				r.setSubtotalEgresos(rol.getSubtotalEgresos().doubleValue());
				r.setSubtotalIngresos(rol.getSubtotalIngresos().doubleValue());
				r.setSubtotalIngresosAdicionales(rol.getSubtotalIngresosAdicionales().doubleValue());
				r.setTotal(rol.getTotal().doubleValue());
			}
			listaDTO.add(e);
		}
		return listaDTO;
	...
	public ThmCargo insertarCargo(String nombreCargo, BigDecimal remuneracion) throws Exception {
        ThmCargo cargo = new ThmCargo();
        cargo.setNombreCargo(nombreCargo);
        cargo.setRemuneracionMensual(new BigDecimal(remuneracion.doubleValue()));
        mDAO.insertar(cargo);
        return cargo;
    ...
    public void actualizarCargo(int getIdThmCargo, String nombre, BigDecimal remuneracion) throws Exception{
        ThmCargo cargoactualizar;
        ThmCargo cargo = (ThmCargo) mDAO.findById(ThmCargo.class, getIdThmCargo);
        cargo.setNombreCargo(nombre);
        cargo.setRemuneracionMensual(remuneracion);
        mDAO.actualizar(cargo);
    }
