...
import minimarketdemo.model.seguridades.dtos.LoginDTOCli;
...
public class ManagerSeguridades {
    ...
    //----------------------------------------------------------------------------------------------------------------
    public LoginDTOCli loginCli(int idCliente,String clave) throws Exception{
    	if(ModelUtil.isEmpty(clave)) {
    		mAuditoria.mostrarLog(getClass(), "login", "Debe indicar una clave "+ idCliente);
    		throw new Exception("Debe indicar una clave");
    	}
    	
    	Cliente cliente=(Cliente) mDAO.findById(Cliente.class, idCliente);
    	if(cliente==null) {
    		mAuditoria.mostrarLog(getClass(), "login", "No existe cliente "+idCliente);
    		throw new Exception("Error en credenciales.");
    	}
    		
    	if(cliente.getClave().equals(clave)) {
    		if(cliente.getActivo()==false) {
        		mAuditoria.mostrarLog(getClass(), "login", "Intento de login cliente desactivado "+idCliente);
        		throw new Exception("El cliente esta desactivado.");
        	}
    		mAuditoria.mostrarLog(getClass(), "login", "Login exitoso "+idCliente);
    		//crear DTO:
    		
    		LoginDTOCli loginDTOCli =new LoginDTOCli();
    		loginDTOCli.setIdCliente(cliente.getIdCliente());
    		loginDTOCli.setCorreo(cliente.getCorreo());
    		//obtener la lista de modulos a los que tiene acceso:
    		
    		List<SegAsignacion> listaAsignaciones=findAsignacionByUsuario(cliente.getIdCliente());
    		for(SegAsignacion asig:listaAsignaciones) {
    			SegModulo modulo=asig.getSegModulo();
    			loginDTOCli.getListaModulos().add(modulo);
    		}
    		return loginDTOCli;
    	}
    	mAuditoria.mostrarLog(getClass(), "login", "No coincide la clave "+idCliente);
    	throw new Exception("Error en credenciales");
    }
    
    public void cerrarSesionCli(int idCliente) {
    	mAuditoria.mostrarLog(getClass(), "cerrarSesion", "Cerrar sesión usuario: "+idCliente);
    }
    
    public void accesoNoPermitidoCli(int idCliente,String ruta) {
    	mAuditoria.mostrarLog(getClass(), "accesoNoPermitido", "Cliente "+idCliente+" intento no autorizado a "+ruta);
    }
    
    public List<Cliente> findAllCliente(){
    	return mDAO.findAll(Cliente.class, "apellidoscliente");
    }
    ...
    public void actualizarCliente(Cliente edicionCliente) throws Exception {
    	...
    	Cliente.setApellidoscliente(edicionCliente.getApellidoscliente());
    	Cliente.setClave(edicionCliente.getClave());
    	Cliente.setCorreo(edicionCliente.getCorreo());
    	Cliente.setCodigo(edicionCliente.getCodigo());
    	Cliente.setNombrescliente(edicionCliente.getNombrescliente());
    	mDAO.actualizar(Cliente);
    }
    
    public void activarDesactivarCliente(int idCliente) throws Exception {
    	Cliente cliente=(Cliente) mDAO.findById(Cliente.class, idCliente);
    	if(idCliente==1)
    		throw new Exception("No se puede desactivar al Cliente administrador.");
    	cliente.setActivo(!cliente.getActivo());
    	System.out.println("activar/desactivar "+cliente.getActivo());
    	mDAO.actualizar(cliente);
    }
